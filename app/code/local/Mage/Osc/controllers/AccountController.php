...
class Mage_Osc_AccountController extends Mage_Customer_AccountController{
	...
	*/
	...
	public function createPostAction()
	{
		...
		if ($this->getRequest()->isPost()) {
			...
			$customer->setTipopessoa($post['tipopessoa']);
			
			if($tipopessoa == 'pj'){
				$ieisento = $post['ieisento'];
				$post['lastname'] = ' - ';
				if($ieisento == 'on'){
					$customer->setIe('Isento');
					$customer->setIeisento(0); // sim
					$customer->setLastname($post['lastname']);
				}else{
					$customer->setIe($post['ie']);
					$customer->setIeisento(1); // nao
					$customer->setLastname($post['lastname']);
				}
