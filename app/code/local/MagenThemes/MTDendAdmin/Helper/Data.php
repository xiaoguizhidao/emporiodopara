<?php
class MagenThemes_MTDendAdmin_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    public function getLabel(Mage_Catalog_Model_Product $product)
    {
        if ( 'Mage_Catalog_Model_Product' != get_class($product) )
            return; 
        $html = '';
        if (!Mage::getStoreConfig("mtdendadmin/product/product_show_newlabel") &&
            !Mage::getStoreConfig("mtdendadmin/product/product_show_salelabel") ) {
            return $html;
        } 
        $this->_loadProduct($product);

        if ( Mage::getStoreConfig("mtdendadmin/product/product_show_newlabel") && $this->_checkNew($product) ) {
            $html .= '<div class="product-new-label new-'.Mage::getStoreConfig('mtdendadmin/product/product_newlabel_positions').'"></div>';
        }
        if ( Mage::getStoreConfig("mtdendadmin/product/product_show_salelabel") && $this->_checkSale($product) ) {
            $html .= '<div class="product-sale-label sale-'.Mage::getStoreConfig('mtdendadmin/product/product_salelabel_positions').'"></div>';
        }
