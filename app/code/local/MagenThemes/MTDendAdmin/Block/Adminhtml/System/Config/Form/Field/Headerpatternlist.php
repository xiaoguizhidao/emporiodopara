<?php
class MagenThemes_MTDendAdmin_Block_Adminhtml_System_Config_Form_Field_Headerpatternlist extends Mage_Adminhtml_Block_System_Config_Form_Field
{
    protected function _getElementHtml(Varien_Data_Form_Element_Abstract $element){ 
       	$html = parent::_getElementHtml($element);
		$directry = Mage::getBaseDir('media').DS.'magenthemes'.DS.'pattern_header'; 
		...
		$html .='<div class="listpattern '.$element->getHtmlId().'_pattern">';
			$html .='<span class="item">
						...
						<input type="radio" name="pattern_header" value="none" style="margin-top: 7px;" class="valptheader"/>
					 </span>';
			if($images){
				foreach ($images as $img){
			$html .='<span class="item">
						<img src="'.$urlparth.'/media/magenthemes/pattern_header/'.$img.'" />
						<input type="radio" name="pattern_header" value="'.$img.'" class="valptheader"/>
					 </span>';
				}
			}
		$html .='</div>';
