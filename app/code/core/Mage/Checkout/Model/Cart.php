    ...
    public function updateItems($data)
    {
        ...
        foreach ($data as $itemId => $itemInfo) {
            ...
            if ($qty > 0) {
                ...
                // Alteração feita por Guilherme Costa - 03/06/2013 e 04/06/2013
                $stockItem =  Mage::getModel('catalog/product')->load($item->getProductId())->getStockItem();
                if($itemInfo['before_suggest_qty'] <= round($stockItem->getQty()) ){
                }else{
                    if ($item->getHasError()) {
                        Mage::throwException($item->getMessage()); 
                    }
