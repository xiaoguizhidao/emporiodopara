    ...
    implements Mage_Shipping_Model_Carrier_Interface
{
    ...
    protected $_alturaSent              = null;
    protected $_larguraSent             = null;
    protected $_comprimentoSent         = null;
    ...
    {       
        ...
        if($this->_generateVolumeWeight($request) === false)
        {
            // Dimension error
    ...
    protected function _getQuotes(){
        ...
        if($correiosReturn !== false){
            ...
            if($existReturn === false){
                // Error on data return Correios
                $this->_validateErrorShipping();
                return $this->_result;
            }
    ...
    /**
     * Make validate error and show a method chosen
     */
    protected function _validateErrorShipping(){
        if($this->getConfigData('alternative_shipping')){
            $this->_apendShippingReturn((string)$this->getConfigData('code_alternative'), $this->getConfigData('value_alternative'), $this->getConfigData('delivery_alternative'));
        }else{
    ...
    protected function _getCorreiosReturn(){
        ...
        try {
            ...
            $client->setParameterGet('nVlComprimento',$this->_comprimentoSent);
            $client->setParameterGet('nVlAltura',$this->_alturaSent);
            $client->setParameterGet('nVlLargura',$this->_larguraSent);
    ...
    protected function _generateVolumeWeight($request){
        ...
        foreach($items as $item){
            ...
            if($_product->getTypeId() == "simple"){
                ...
                $this->_alturaSent += $itemAltura * $item->getQty();
                $this->_larguraSent += $itemLargura * $item->getQty();
                $this->_comprimentoSent += $itemComprimento * $item->getQty();

            }
        }

        if($this->_alturaSent < $this->getConfigData('volume_validation/altura_min'))
            $this->_alturaSent = $this->getConfigData('volume_validation/altura_min');

        if($this->_larguraSent < $this->getConfigData('volume_validation/largura_min'))
            $this->_larguraSent = $this->getConfigData('volume_validation/largura_min');

        if($this->_comprimentoSent < $this->getConfigData('volume_validation/comprimento_min'))
            $this->_comprimentoSent = $this->getConfigData('volume_validation/comprimento_min');

        /**
        Tarefa: 23424, setando valor maximo quando os produtos do carrinho ultrapassarem o limite (Começo)
        */

        if($this->_alturaSent > $this->getConfigData('volume_validation/altura_max'))
            $this->_alturaSent = $this->getConfigData('volume_validation/altura_max');

        if($this->_larguraSent > $this->getConfigData('volume_validation/largura_max'))
            $this->_larguraSent = $this->getConfigData('volume_validation/largura_max');

        if($this->_comprimentoSent > $this->getConfigData('volume_validation/comprimento_max'))
            $this->_comprimentoSent = $this->getConfigData('volume_validation/comprimento_max');

        if(($this->_alturaSent + $this->_larguraSent + $this->_comprimentoSent) > $this->getConfigData('volume_validation/sum_max')){
            $this->_alturaSent = 66; $this->_larguraSent = 66; $this->_comprimentoSent = 66;
        }

        /**
        Tarefa: 23424, setando valor maximo quando os produtos do carrinho ultrapassarem o limite (Fim)
        */
        ...
        if(
            ...
            || $this->_larguraSent > $this->getConfigData('volume_validation/largura_max')
            || $this->_comprimentoSent > $this->getConfigData('volume_validation/comprimento_max')
            || ($this->_alturaSent + $this->_larguraSent + $this->_comprimentoSent) > $this->getConfigData('volume_validation/sum_max')
        ){
            return false;
        }
